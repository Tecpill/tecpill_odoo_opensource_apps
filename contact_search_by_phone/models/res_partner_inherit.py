...
class ResPartnerInherit(models.Model):
    ...
    def _compute_phone_normalized(self):
        ...
        for partner in self:
            ...
            # Normalize mobile
            mobile_normalized = self._normalize_phone(partner.mobile, strip_leading_zero=strip)
            ...
            if partner.mobile_clean:
                ...
                if duplicate:
                    ...
                    raise ValidationError(
                        ...
                        (partner.mobile, duplicate.display_name, field_name)
                    )

    def name_get(self):
        """
        Enhance display: Add phone number in search results
        Example: "Ahmed Mohamed — 0551234567"
        Automatically includes phone if show_phone_in_search config is enabled
        """
        result = []
        
        # Check if show_phone_in_search is enabled in settings
        show_phone_config = self.env['ir.config_parameter'].sudo().get_param(
            'contact_search_by_phone.show_phone_in_search', default='True'
        )
        show_phone = tools.str2bool(show_phone_config) or self.env.context.get('show_phone_in_name', False)
    ...
    def _name_search(self, name='', args=None, operator='ilike', limit=100, name_get_uid=None, order=None):
        """
        Smart advanced search with priority system (Odoo 18 Compatible):
        ...
        Args:
            ...
            name_get_uid: User ID for permission check (Odoo 18)
